...
def medal_tally(df) :
    ...
    medal_tally = medal_tally.groupby('NOC')['Gold', 'Silver', 'Bronze'].sum().sort_values('Gold',
...
def country_year_list(data) :
    ...
    country = list(np.unique(list(data['region'].dropna().values)))
    country.sort()
...
def participating_nations_over_time(data, col) :
    # dropping the rows with same year and region entry and then calculating the number of countries in every edition and sorting them acc. to year
    nations_over_time = data.drop_duplicates(['Year', col])['Year'].value_counts().reset_index().sort_values('index')
...
def most_successful(df, sport) :
    ...
    x = x.rename(columns = {'index' : 'Athlete', 'Name_x' : 'Medals'})
    x = x.reset_index()
    return x[['Athlete', 'Medals', 'Sport', 'region']]
...
def most_successful_from_country(df, country) :
    temp_df = df.dropna(subset=['Medal'])
...
def men_vs_women(df, sport):
    if sport != 'Overall' :
        athlete_df = df[df['Sport'] == sport]
        athlete_df = athlete_df.drop_duplicates(subset=['Name', 'region'])
    else :
        athlete_df = df.drop_duplicates(subset=['Name', 'region'])
